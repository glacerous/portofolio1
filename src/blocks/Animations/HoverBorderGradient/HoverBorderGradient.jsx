...
export default function HoverBorderGradient({ children, containerClassName, className, as: Tag = 'button', duration = 1, clockwise = true, ...props }) {
  ...
  const highlight = 'radial-gradient(75% 181.15942028985506% at 50% 50%, #3643FC 0%, rgba(255, 255, 255, 0) 100%)';
  ...
  return (
    <Tag
      ...
      className={cn('relative flex h-min w-fit flex-col flex-nowrap content-center items-center justify-center gap-10 overflow-visible bg-black/20 decoration-clone p-px transition duration-500 hover:scale-105 hover:bg-black/10', containerClassName)}
      {...props}
    >
      <div className={cn('z-10 w-auto rounded-[inherit] bg-black py-2', className)}>{children}</div>
      <motion.div
